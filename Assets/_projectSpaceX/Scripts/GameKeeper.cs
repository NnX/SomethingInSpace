...
namespace _projectSpaceX.Scripts
{
    public class GameKeeper : MonoBehaviour
    {
        ...
        private void InitEnemies()
        {
            ...
            for (var i = 0; i < enemyRowsAmount; i++)
            {
                ...
                for (var j = 0; j < enemyColumnsAmount; j++)
                {
                    var enemyObject = Instantiate(enemies[randomEnemyIndex], columnPosition, quaternion.identity, startTransform);
                    enemyObject.gameObject.transform.SetParent(rowLayout.transform);
                    ...
                    columnPosition.x = columnPosition.x + ((RectTransform)enemyObject.transform).sizeDelta.x + spaceBetweenEnemies;
                }
                ...
                columnPosition.y -= (MaxEnemyHeight + spaceBetweenRows);
                columnPosition.x = 0;
            }
        }
        ...
        private void MoveEnemyRowsOnAxisX()
        {
            ...
            {
                if (_enemyRowsList[^1].TryGetComponent<EnemyRow>(out var enemyRow))
                {
                    enemyRow.SetLastRow(true);
                }
            }
